...
public class HUD : MonoBehaviour
{
    ...
    public void SetScore(int score)
    {
        ...
        int visibleStar = 0;
        if (score >= level.socore1Star && score < level.socore2Star)
        {
            visibleStar = 1;
        }
        else if (score >= level.socore2Star && score < level.socore3Star)
        {
            visibleStar = 2;
        }
        else if (score >= level.socore3Star)
        {
            visibleStar = 3;
        }
        for (int i = 0; i < stars.Length; i++)
    ...
    public void SetLevelType(Level.LevelType type)
    {
        if (type == Level.LevelType.MOVES)
        {
            ...
            targetSubtext.text = "target socre";
        }
